        ...
        <div class="container d-flex align-items-center px-md-0">
            ...
            <div class="collapse navbar-collapse justify-content-center mobile-offcanvas " id="navbarNav">
                ...
                <ul class="navbar-nav">
                    ...
                    <li class="nav-item">
                        <a class="nav-link @if (Route::is('find_legal_help')) active @endif" href="{{ route('find_legal_help') }}">Find Legal Help</a>
                    </li>
                    <li class="nav-item">
                        <a class="nav-link @if (Route::is('services')) active @endif" href="{{ route('services') }}">Services</a>
                    </li>
                    <li class="nav-item">
                        <a class="nav-link @if (Route::is('agency')) active @endif" href="{{ route('agency') }}">Agency</a>
                    </li>
                    <li class="nav-item">
                        <a class="nav-link @if (Route::is('contact_us')) active @endif" href="{{ route('contact_us') }}">Contact Us</a>
                    </li>
                </ul>
                @endif
            </div>
            <div class="d-flex gap-3 align-items-center right_dec_text">
                @if (Route::has('login'))
                    <div>
                        @auth
                            <div class="dropdown">
                                <a href=" @if(Auth::user()->roles()->first()->id == 2) {{ route('lawyer.dashboard') }} @else {{ route('client.dashboard') }}  @endif" class="dropdown-toggle" type="button"
                                    id="dropdownMenuButton1" data-bs-toggle="dropdown" aria-expanded="false">
                                    {{ Auth::user()->name }}
                                </a>
                                <ul class="dropdown-menu db_head_dropdown dropdown-menu-end" aria-labelledby="dropdownMenuButton1">

                                    <li>
                                        @if(Auth::user()->roles()->first()->id == 2)
                                        <a class="dropdown-item @if (Route::is('lawyer.dashboard')) active @endif" href="{{ route('lawyer.dashboard') }}">Dashboard</a>
                                        @else
                                        <a class="dropdown-item @if (Route::is('client.dashboard')) active @endif" href="{{ route('client.dashboard') }}">Dashboard</a>
                                        @endif
                                    </li>
