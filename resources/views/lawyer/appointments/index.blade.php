    ...
    <div class="row flex-nowrap">
        ...
        <div class="col col-xl-9 py-lg-5 py-4 right_width">
                            ...
                            <td>
                                <div class="dropdown pl_action_drop">
                                    ...
                                    <ul class="dropdown-menu" aria-labelledby="dropdownMenuButton1" style="width:168px">
                                        <li><a href="{{ route('lawyer.appointment.detail', ['id' => isset($appointment->id) ? $appointment->id : '']) }}" class="text-dark px-3 py-2">View Detail</a> </li>
                                        <?php
                                        ...
                                        <li><button type="button" class="text-dark px-3 py-2 reschedule" data-bs-toggle="modal" data-bs-target="#rescheduleModal" data-id="{{ isset($appointment->id) ? $appointment->id : '' }}" data-date="{{ isset($appointment->date) ? $appointment->date : '' }}" data-start="{{ isset($appointment->start_time) ? date_format($appointment->start_time, 'H:i') : '' }}" data-end="{{ isset($appointment->end_time) ? date_format($appointment->end_time, 'H:i') : '' }}">Reschedule</button> </li>
                                        @endif
                                        @endif
                                        @if (isset($appointment->status) && $appointment->status == 3)
                                        <li><a href="{{ isset($appointment->start_url) ? $appointment->start_url : '' }}" class="text-dark px-3 py-2" target="_blank">Start</a> </li>
                                        @endif
                                        ...
                                        <li>
                                            ...
                                            <form action="{{ route('lawyer.appointment.update_status') }}" method="POST">
                                                ...
                                                <button type="submit" name="status" value="2" class="text-dark px-3 py-2">
                                                    Accept
                                                </button>
                                            </form>
                                            @else
                                            <a href="{{ route('lawyer.get_hourly_rate') }}" class="text-dark px-3 py-2" id="set_rate_accept">Accept</a>
                                            @endif
                                        </li>
                                        @endif
                                        @if (isset($appointment->reshedule_status) && $appointment->reshedule_status == 1 && !$appointment->isCancelled())
                                        <li><button type="button" class="text-dark px-3 py-2 rescheduleBtn" data-id="{{ isset($appointment->id) ? $appointment->id : '' }}">
                                                Reschedule Accept
                                            </button> </li>
                                        @endif
                                        @if (isset($appointment->status) && ($appointment->status == 3 || $appointment->status == 6) &&
                                        ...
                                        @else
                                        <li> <a href="{{ route('lawyer.appointment.cancel_request', ['id' => isset($appointment->id) ? $appointment->id : '']) }}" class="text-dark px-3 py-2">Delete</a> </li>
                                        @endif
                                        @php
                                        $currentTime = now('UTC');
                                        ...
                                        <li> <a href="{{ route('lawyer.appointment.send_invoice', ['id' => isset($appointment->id) ? $appointment->id : '']) }}" class="text-dark px-3 py-2">Invoice</a> </li>
                                        @endif
                                    </ul>
                                </div>
            ...
            <td>
                ...
                <span class="pl_badge @if(!empty($appointment->payment_id)) pl_bg_success @else pl_bg_primary_dark @endif">@if(!empty($appointment->payment_id))Confirm and @endif Reschedule Requested</span>
                @else
                <span class="pl_badge @if(!empty($appointment->payment_id)) pl_bg_success @else pl_bg_primary_dark @endif">@if(!empty($appointment->payment_id))Confirm and @endif Resheduled</span>
                @endif
                @else
            ...
            <form action="{{ route('lawyer.appointment.check') }}" method="POST" id="reschedule_form2">
                ...
                <div class="modal-header">
                    <h4 class="modal-title">Reschedule appointment
                        request</h4>
                    <button type="button" class="close border-0 fs-4 bg-none" data-bs-dismiss="modal">&times;</button>
                </div>
                <!-- Modal body -->
                <div class="modal-body container p-0 border-0">
                    <div class="row">
                        <input type="hidden" name="date" id="reschedule_date" min="{{ date('Y-m-d') }}" value="">
                ...
                <div class="modal-footer justify-content-center">
                    ...
                    <button type="button" class="btn btn-danger rejectBtn" data-bs-dismiss="modal">Reject</button>
                </div>
            </form>
        </div>
    </div>
