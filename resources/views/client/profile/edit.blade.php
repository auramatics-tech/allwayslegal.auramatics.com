        ...
        <div class="row flex-nowrap">
            <div class="col py-lg-5 py-4 right_width right_width">
                ...
                <div class="mb-5" style="background:#fff">
                    <form method="POST" action="{{ route('client.profile.update', isset($client->id) ? $client->id : '') }}" class="profile_form">
                        @method('PATCH')
                        @csrf
                        <div class="card-body p-4 default_card">
                            <div class="row">
                                ...
                                <div class="col-md-6 mb-3">
                                    ...
                                    <select name="country" class="form-select form-control-item form-control @error('country') is-invalid @enderror mb-0" style="padding:11px">
                                        <option value="">-- Select Country--</option>
                                        @if(count($countries))
                                        @foreach($countries as $country)
                                            <option value="{{ $country->id }}" {{ isset($client->country) && $client->country == $country->id ? 'selected' : '' }}>
                                                {{ $country->name }}
                                            </option>
                                ...
                                <div class="col-md-6 mb-3">
                                    ...
                                    <select name="province" class="form-select form-control-item form-control @error('province') is-invalid @enderror mb-0" style="padding:11px">
                                        <option value="">-- Select Province--</option>
                                        @if(count($provinces))
                                        @foreach($provinces as $province)
                                            <option value="{{ $province->id }}" {{ isset($client->province) && $client->province == $province->id ? 'selected' : '' }}>
                                                {{ $province->name }}
                                            </option>
                                        @endforeach
                                        @endif
                                ...
                                <div class="col-md-6 mb-3">
                                    ...
                                    <select name="city" class="form-select form-control-item form-control @error('city') is-invalid @enderror mb-0" style="padding:11px">
                                        <option value="">-- Select City--</option>
                                        @if(count($cities))
                                        @foreach($cities as $city)
                                            <option value="{{ $city->id }}" {{ isset($client->city) && $client->city == $city->id ? 'selected' : '' }}>
                                                {{ $city->name }}
                                            </option>
                                        @endforeach
                                        @endif
                            ...
                            <div class="text-center d-flex justify-content-center gap-3 mt-4">
                                <div class="btn-style"><a href="{{ route('client.dashboard') }}"
