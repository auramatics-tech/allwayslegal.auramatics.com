(function(e) { e.fn.calendarJs = function (options) {
	...
	function initMonthNames() {
		var monthNames = [];
		monthNames.push({"fr": "Janvier", "en": "Jan"});
		monthNames.push({"fr": "Février", "en": "Feb"});
		monthNames.push({"fr": "Mars", "en": "Mar"});
		monthNames.push({"fr": "Avril", "en": "Apr"});
		monthNames.push({"fr": "Mai", "en": "May"});
		monthNames.push({"fr": "Juin", "en": "Jun"});
		monthNames.push({"fr": "Juillet", "en": "Jul"});
		monthNames.push({"fr": "Aout", "en": "Aug"});
		monthNames.push({"fr": "Septembre", "en": "Sep"});
		monthNames.push({"fr": "Octobre", "en": "Oct"});
		monthNames.push({"fr": "Novembre", "en": "Nov"});
		monthNames.push({"fr": "Décembre", "en": "Dec"});
		return monthNames;
	}
    ...
    class Month {
        ...
        asTable(year, lang) {
			...
			container.append('<div class="flex-container"><button type="button" class="btn prev-month"> <i class="fa fa-chevron-left p-1"></i> </button>' +
			'<span style="flex: 1; display:none;"></span><h1> '+ monthName + '-' + year + '</h1><button type="button" class="btn next-month"> <i class="fa fa-chevron-right p-1"></i> </button></div>');
            container.append();
            ...
            for (var iDay = 0; iDay < 7; iDay++) {
                tr.append("<th>" + shortenName(getDayName(iDay, lang), 3) + "</th>");
            }
            ...
            for (iDay = 0; iDay < this.days.length; iDay++) {
                ...
                var dayCompleteDescription = ('0'+this.days[iDay].dayInMonth).slice(-2) + " " + monthName + " " + year;

                var classOfDay = dayIsDate(this.days[iDay].date, now) ? "date_td now" : "date_td";
